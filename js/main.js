...
(function () {
    ...
    let carousel = document.querySelectorAll('carousel'), // Контейнер карусели
        cardList = document.querySelectorAll('.card-list')[0], // Список ul с карточками
        cards = document.querySelectorAll('.card-item--sets'), // Псевдомасив карточек
        dotsSets = document.querySelectorAll('.dots__item--sets'), // Псевдомассив точек
...
// Сдвиг карточек вправо при нажатии на левую стрелку
    prev.addEventListener('click', () => {

        if (activeDot > 0) {
            activeDot -= 1;
            dotsSets[activeDot + 1].classList.remove('dots__item--active');
            dotsSets[activeDot].classList.add('dots__item--active');
            position = Math.min(position + cardWidth * cardCount, 0)
            cardList.style.marginLeft = `${position}px`;
        } 
        console.log(dotsSets[activeDot]);
        console.log(activeDot);
    });

// Сдвиг карточек влево при нажатии на правую стрелку
    next.addEventListener('click', () => {

        if (activeDot < dotsSets.length - 1) {
            activeDot += 1;
            dotsSets[activeDot - 1].classList.remove('dots__item--active');
            dotsSets[activeDot].classList.add('dots__item--active');
            position = Math.max(position - cardWidth * cardCount, -cardWidth * (cards.length - cardCount));
            cardList.style.marginLeft = `${position}px`;
        } 
        console.log(dotsSets[activeDot]);
        console.log(activeDot);
